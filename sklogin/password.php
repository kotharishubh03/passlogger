<?php
    ...
    $stmt = $pdo->prepare('SELECT * FROM `password` where `usr_id` in (SELECT usr_id FROM `sess_manager` WHERE `session_id` = :sessid)');
		$stmt->execute(array( ':sessid' => $_SESSION['sessionid'],));
		$row = $stmt->fetchall();
		if ( $row !== false ) {
			//print_r($row);
                        ...
                        echo('<div class="w3-col m4 s6 w3-padding ">
                            <div class="w3-card-4 w3-dark-grey w3-round-xlarge">
                                <div class="w3-container w3-center w3-hover-shadow">
                                    ...
                                    <h6>'.$r["username"].'</h6>
                                    <img class="w3-round-xlarge" src="https://img.pngio.com/password-computer-icons-clip-art-coheed-and-cambria-symbol-png-password-png-900_980.jpg" alt="Avatar" style="width:60%">
